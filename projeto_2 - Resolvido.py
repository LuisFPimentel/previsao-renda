...
# Adicionando um filtro na sidebar
min_renda = st.sidebar.number_input('Valor Mínimo de Renda', min_value=float(renda['renda'].min()), max_value=float(renda['renda'].max()), value=float(renda['renda'].min()))
max_renda = st.sidebar.number_input('Valor Máximo de Renda', min_value=float(renda['renda'].min()), max_value=float(renda['renda'].max()), value=float(renda['renda'].max()))

# Aplicando os filtros
renda = renda[(renda['renda'] >= min_renda) & (renda['renda'] <= max_renda)]


# Criando links para os gráficos na sidebar
st.sidebar.write("[Gráficos ao longo do tempo](#graficos-ao-longo-do-tempo)")

# Adicionando uma marcação na página para servir como âncora
st.markdown("<a name='graficos-ao-longo-do-tempo'></a>", unsafe_allow_html=True)
...
# Criando links para os gráficos na sidebar
st.sidebar.write("[Gráficos bivariados](#graficos-bivariados)")

# Adicionando uma marcação na página para servir como âncora
st.markdown("<a name='graficos-bivariados'></a>", unsafe_allow_html=True)
